...
$stmt = $db->prepare('SELECT * FROM activerapid WHERE winall = :winall');
...
foreach ($result as $row) { ?>
   ...
   <?php $stmt1 = $db->prepare('SELECT * FROM activerapid WHERE isdone = :isdone AND isadded = :isadded AND gplayer = :gplayer ORDER BY id ASC');
	...
	<?php	} else { ?>
		...
		<?php } ?>
									...
									<i class="booked-icon booked-icon-clock"></i><strong><?php echo $row1['garesults']; ?> <?=gettext("secounds");?></strong> <?=gettext("on");?> <?php echo $rounds;?> <?=gettext("hits");?>
		...
		<div class="section-heading section-heading--divider-top section-heading--left">
				...
				<div class="section-heading section-heading--divider-top section-heading--right">
					...
					<a href="#" onclick="printGame()" id="printGame" class="btn btn-default btn-sm btn-outline-secondary cancel"><?=gettext('Print')?></a>
		...
		jQuery.ajax({
            ...
            success: function(data) {
                ...
                if (fel == "true") {
                    ...
                    alert('<?=gettext('Starting printing, let the print be done before you exit the game.');?>');
                }
            }
        });
	}
