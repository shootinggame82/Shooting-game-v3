...
<head>
	...
	<title><?=gettext('Battery status')?></title>
	<meta charset="utf-8">
					...
					<div class="col-lg-8 ml-lg-auto mr-lg-auto">
						...
						<div id="booked-profile-page" class="booked-shortcode box">
							<div class="booked-profile-appt-list">
								<h4><?=gettext('Battery status for targets')?></h4>
							<?php
$stmt = $db->prepare('SELECT * FROM targets WHERE batok = :isok');
$stmt->execute(array(':isok' => '1'));
$result = $stmt->fetchAll();
foreach ($result as $row) { 
		$voltres = $row['batstatus'] / 1000;
	?>
		
		
			<span class="appt-block bookedClearFix approved">
			<span class="status-block"><i class="booked-icon booked-icon-radio-checked"></i>
			<span><?php echo $voltres; ?> <?=gettext('Volt')?></span></span>
	
									 
									<i class="booked-icon booked-icon-clock"></i><strong><?=gettext("Name on target:");?></strong> <?php echo $row['tarname'];?>
	...
	<?php }?>
				...
				<div class="section-heading section-heading--divider-top section-heading--right">
					...
					<a href="#" onclick="quitBat()" id="quitBat" class="btn btn-primary btn-sm"><?=gettext('Back')?></a>
