...
$stmt = $db->prepare('SELECT * FROM activetimed');
$stmt->execute();
...
foreach ($result1 as $row1) {
    ...
    $stmt = $db->prepare('INSERT INTO savedtimed (garesults,isdone,isadded,gplayer,winusr,winall,gamecode,fullname,gametype,gamedate, gametime) VALUES (:garesults, :isdone, :isadded, :gplayer, :winusr, :winall, :gamecode, :fullname, :gametype, :gamedate, :gametime)');
    $stmt->execute(array(
...
$stmt = $db->prepare('DELETE FROM activeplayers');
    $stmt->execute();
    $stmt = $db->prepare('DELETE FROM activetimed');
    $stmt->execute();
