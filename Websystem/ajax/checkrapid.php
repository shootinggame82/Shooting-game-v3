...
$stmt = $db->prepare('SELECT * FROM activerapid WHERE isdone = :isdone AND isadded = :isadded AND gplayer = :gplayer');
...
//Mark as added
try {
    $stmt = $db->prepare('UPDATE activerapid SET isadded = :isadded WHERE id = :id');
    $stmt->execute(array(
':isadded' => '1',
':id' => $roundid
));
$echodata = array('error' => 'false', 'errorcode' => '0', 'error1' => '', 'result' => $results, 'done' => '1');
echo json_encode($echodata);
} catch (PDOException $e) {
    $error[] = $e->getMessage();
$echodata = array('error' => 'true', 'errorcode' => '1', 'error1' => $e->getMessage(), 'result' => '', 'done' => '1');
echo json_encode($echodata);
}
} else {
    $echodata = array('error' => 'false', 'errorcode' => '0', 'error1' => '', 'result' => '', 'done' => '0');
}
