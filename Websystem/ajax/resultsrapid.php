...
//Now winning player
$stmt1 = $db->prepare('SELECT * FROM activerapid WHERE garesults = (SELECT MIN(garesults) FROM activerapid WHERE isdone = :isdone AND isadded = :isadded)');
    $stmt1->execute(array(':isdone' => '1',
        ':isadded' => '1'));
    $result1 = $stmt1->fetchAll();
    foreach ($result1 as $row1) {
        $stmt = $db->prepare('UPDATE activerapid SET winall = :winall WHERE id = :id');
        $stmt->execute(array(
            ':winall' => '1',
            ':id' => $row1['id'],
        ));
