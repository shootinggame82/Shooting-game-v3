...
$stmt = $db->prepare('SELECT * FROM activetimed WHERE winall = :winall');
...
foreach ($result as $row) { ?>
   ...
   <?php $stmt1 = $db->prepare('SELECT * FROM activetimed WHERE isdone = :isdone AND isadded = :isadded AND gplayer = :gplayer ORDER BY id ASC');
	...
	<?php	} else { ?>
		...
		<?php } ?>
									...
									<i class="booked-icon booked-icon-clock"></i><strong><?php echo $row1['garesults']; ?> <?=gettext("hits");?></strong> <?=gettext("on");?> <?php echo $rounds;?> <?=gettext("secounds");?>
		...
		jQuery.ajax({
            ...
            success: function(data) {
                ...
                if (fel == "true") {
                    ...
                    alert('<?=gettext('Starting printing, let the print finish before you exit the game.');?>');
                }
            }
        });
	}
